...
protected:
    ...
    std::atomic_bool m_sealerListUpdated = {true};//update-ConsensusNodeList()里更新
    int64_t m_lastSealerListUpdateNumber = 0;//最近更新的sea-lerList所在的区块号
    /// p2p service handler
    ...
    //本轮共识区块号，=m_highes-tBlock+1
    std::atomic<int64_t> m_consensusBlockNumber = {0};
    /// the latest block header 当前轮共识的链上最高区块头
    ...
    dev::h512s m_sealerList;//用当前轮的共识节点来赋值，从m_-blockChain->sealerList()取出来更新

    /// allow future blocks or not
    bool m_allowFutureBlocks = true;
    bool m_startConsensusEngine = false; //标记共识是否开始

    /// node list record when P2P last update
    std::string m_lastNodeList;//最新的节点列表，"[updateConsensusNodeList] Sealers:"..."Observers:"...
...
// only when supported_version>=v2.6.0, support adjust consensus interval at runtime ledger.cpp 400行
    bool m_supportConsensusTimeAdjust = false;//不考虑
