    ...
    try
    {
        ...
        std::sort(sealerList.begin(), sealerList.end());
        {
            ...
            if (sealerList != m_sealerList)
            {
                ...
                m_sealerListUpdated = true;//rpbft
                //当创世块时，m_b-lockChain->number();返回什么？？？0吗？ zhuangql
                //也就是创世块的区块高度是0还是1，如果是0刚启动时m_b-lockChain->number()返回什么？
                //m_lastSealerListUpdateNum-ber2。0。0没有，//rpbft
                m_lastSealerListUpdateNumber = m_blockChain->number();
            }
            else if (m_blockChain->number() != m_lastSealerListUpdateNumber)//rpbft
            {
                m_sealerListUpdated = false;//rpbft
            }
        ...
        if (m_lastNodeList != s2.str())//返回临时的string对象
        {
            ...
            //更新P2P的节点列表  内部是怎么管理列表的 ？？？ zhuangql
...
//2。0。0的注释// recalculate m_nodeNum && m_f && m_cfgErr(must called after setSigList)
//向区块添加signList后，调用来配置环境
void ConsensusEngineBase::resetConfig()
 //从系统配置（不是配置文件）中更新enginebase环境
{//1、更新区块最大交易数
//2、更新当前轮的共识节点列表，包括sealerList，p2p节点列表，NodeList
//3、更新本节点的类型 Sealer Observer
//4、节点总数、m_f、m_cfgErr、m_idx
    updateMaxBlockTransactions();//从blockchain系统配置中取出最大交易数量来更新 //单元测试时不执行
    auto node_idx = MAXIDX;
    m_accountType = NodeAccountType::ObserverAccount;
    size_t nodeNum = 0;
    updateConsensusNodeList(); //单元测试时不执行
    {
        ReadGuard l(m_sealerListMutex);
        //当前节点是否在sealer列表中，给当前节点排序标号
        for (size_t i = 0; i < m_sealerList.size(); i++)
        {
            if (m_sealerList[i] == m_keyPair.pub())
            {
                //Observers节点node_idx == MAXIDX
    ...
    m_idx = node_idx; //此处赋值的midx，在pbft bind p2p时会用到
}
