...
public:
    ...
    virtual void addRawPrepare(PrepareReq::Ptr req)
    {
        ...
        m_rawPrepareCache = req;//增加新的rawPrepare应该清理prepare cache
...
protected:
    ...
    {//效果是清空缓存中 最高区块以外的 所有缓存消息
        for (auto it = cache.begin(); it != cache.end();)
        {
            ...
            if (it->second.size() == 0)//如果某一区块的签名消息为空，则删除这个区块的缓存
                it = cache.erase(it);
            else
                it++;//否则遍历下一个区块hash
        }
...
protected:
    ...
    //2。缓存交易执行结果。区块的交易合法，则缓存
    //prepare内的区块不是空块，执行完区块并产生 signReq 签名请求后，加入到此prepareCache缓存和signReq缓存
    PrepareReq::Ptr m_prepareCache;
    /// cache for raw prepare request 
    //1。prepare包消息合法，则缓存。缓存合法的Prepare包，若Prepare请求合法，则加入rawprepare缓存。
    //合法性判断：主要判断是否是重复的Prepare包、Prepare请求中包含的区块父哈希
    //是否是当前节点最高块哈希(防止分叉)、Prepare请求中包含区块的块高是否等于最高块高加一；
    //用于过滤重复的Prepare请求
    //1。1空块判断：若Prepare请求包含的区块中交易数目是0，则触发空块视图切换。若不为空，执行区块，并缓存

    //只有验证prepare消息的区块是有效的后才赋值更新
    //触发viewchange时clear
    ...
    //只能缓存未来的一个区块，如果处理性能慢，有两个未来块的signReq，按照放弃处理？？？
    //应该有机制追赶上最高区块，//未来块可以有多个
    //h256为区块的hash，SignReq为对区块hash的 签名，string表示区块hash签名的16进制字符串
    //会chche未来块的sign签名
    std::unordered_map<h256, std::unordered_map<std::string, SignReq::Ptr>> m_signCache;
    //3。1若收集满签名包，备份pre-prepare阶段缓存的Prepare包落盘：为了防止Commit阶段区块未提交
    //到数据库之前超过2*f+1个节点宕机，这些节点启动后重新出块，导致区块链分叉(剩余的节点最新区块
    //与这些节点最高区块不同)，还需要备份pre-prepare阶段缓存的Prepare包到数据库，节点重启后，
    //优先处理备份的Prepare包。
    //？？？落盘这里要看一下论文 zhuangql
    ...
    //判断ViewChange包是否有效: 有效的ViewChange请求中带有的块高值必须不小于当前节点最高块高，
    //视图必须大于当前节点视图；
    //viewtype为将要切换的视图，IDXTYPE为节点的index，iewchangereq为对应节点的iewchange请求
    std::unordered_map<VIEWTYPE, std::unordered_map<IDXTYPE, ViewChangeReq::Ptr>>
        m_recvViewChangeReq;
    // only record the latest view of all the nodes
    //不明白？？？ zhuangql   2.2版本没有
    ...
    //4。commit请求合法，则缓存
    //合法性判断：主要判断Commit包的哈希与pre-prepare阶段缓存的执行后的区块哈希相同，
    //若不相同，则继续判断该请求是否属于未来块Commit请求(产生未来块的原因是本节点处理性能低于其他节点，
    //还在进行上一轮共识，判断未来块的条件是：Commit的height字段大于本地最高块高加一)，若请求也非未来块，
    //则是非法的Commit请求，节点直接拒绝该请求；
    ...
    //只在每轮共识的prepare阶段完成时才会赋值更新一次
    //对区块的prepare收到足够的signReq时，将prepare赋值给此变量
    //因为所有可信节点对高度h分配的block达成共识，所以要记录
    PrepareReq m_committedPrepareCache;
    mutable SharedMutex x_committedPrepareCache;
    /// cache for the future prepare cache
    /// key: block height, value: the cached future prepeare
