...
// PBFTMsgPacket for ttl-optimize                      默认打开 ，可关闭     PBFT消息转发优化策略    pbftEngine里的 m_enableTTLOptimize 
...
struct PBFTMsg
{
    ...
    virtual void encode(bytes& encodedBytes) const
    {
        ...
        streamRLPFields(tmp);
        RLPStream list_rlp;
        list_rlp.appendList(1).append(tmp.out());//.appendList(1)不是很懂 rlp
        list_rlp.swapOut(encodedBytes);
    }
     ...
     */
    virtual void decode(bytesConstRef data, size_t const& index = 0)//没全懂
    {
        RLP rlp(data);
        populate(rlp[index]);
    }
    ...
    virtual void populate(RLP const& rlp)                                           //没全懂
    {
        int field = 0;
        try
        {
...
int partition(std::vector<int> &data, int start, int end){
    ...
    int small_maxIndex = start - 1;

    for(size_t index = start; index < end; index++){
        if(data[index] < data[end]){
            small_maxIndex++;
            std::swap(&data[small_maxIndex], &data[index])
        }
    }
    small++
