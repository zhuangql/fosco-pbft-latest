...
std::shared_ptr<Sealer> Ledger::createPBFTSealer()
{
    ...
    //用engine初始化sealer，在用sealer的engine和成员函数初始化engine环境
    //主要初始化engine的可调用成员和sealerList
    ...
    //从配置文件用pbftsealer初始化其中的engine，主要是timer相关
...
//4、设置ttl
    ...
    //从配置文件设置空块产生时间，超过这个时间产生空块
    pbftEngine->setEmptyBlockGenTime(g_BCOSConfig.c_intervalBlockTime);
    //考虑到PBFT模块打包太快会导致某些区块中仅打包1到2个很少的交易，浪费存储空间
    //PBFT共识打包的最短时间，即：共识节点打包时间超过min_block_generation_time
    //且打包的交易数大于0才会开始共识流程，处理打包生成的新区块。
    //默认为500ms共识节点最长打包时间为1000ms，
//若超过1000ms新区块中打包到的交易数仍为0，共识模块会进入出空块逻辑，空块并不落盘；
...
bool Ledger::consensusInitFactory()
{
    ...
    // create PBFTSealer
             ...
             normalrPBFTEnabled() || vrfBasedrPBFTEnabled())
    {
        //pbftSealer含有已经初始化过的engine，用配置文件参数初始化sealer和engine
    ...
    //和同步模块有关，2。0。0没有 zhuangql
    m_sealer->consensusEngine()->setNodeTimeMaintenance(m_nodeTimeMaintenance);
    // create blockFactory 创造block工厂，生产区块
    auto blockFactory = createBlockFactory();
    //用block工厂初始化 sealer中的sealing和engine
    m_sealer->setBlockFactory(blockFactory);//共识引擎和sealing有同一个区块工厂
    return true;
}
